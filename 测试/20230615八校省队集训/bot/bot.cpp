...
bool operator == (const node &a, const node &b){return (a.p == b.p && a.f == b.f);}
...
signed main(){
    ...
    //     if(a.X == b.X) mp.Modify((node){a, 0}, b), mp.Modify((node){b, 2}, a);
    //     else mp.Modify((node){a, 1}, b), mp.Modify((node){b, 3}, a);
    ...
    //     uf(i, 0, 4){
    //         PII pp = p;
    //         while(mp.Exist((node){pp, i}) && mp.Exist((node){mp.Query((node){pp, i}), i})){
    //             PII tp = mp.Query((node){pp, i});
    //             mp.Modify((node){pp, i}, mp.Query((node){tp, i}));
    ...
    // while(true){
    //     bool flag = false;
    //     uf(i, 0, 3) if(mp.Exist((node){v[pp], i})){flag = true; break;}
    //     if(flag) break;
    //     ++ pp;
	// }
	// PII p = v[pp];
    ...
    //     bool flag = false;
    //     for(int i = 0; i < 3; ++ i)
    //         if(mp.Exist((node){p, i}) && !vis.Query((node){p, i}) && !vis.Query((node){mp.Query((node){p, i}), (i + 2) % 4})){
    //             vis.Modify((node){p, i}, true);
    //             p = mp.Query((node){p, i});
    //             vis.Modify((node){p, (i + 2) % 4}, true);
    //             flag = true;
    //             break;
    //         }
    //     if(!flag) break;
